...
Notes:
- JSON files under `backend/` are the application’s local data stores (suitable for dev/testing and CI). In production, back a real database/cloud store.
...
- Store evaluation results into `question_evaluations.json`
- Record teacher feedback via `/api/teacher-feedback` into `teacher_feedback_records.json`
...
- Responsibilities:
  ...
  - Persist evaluation results to `question_evaluations.json` with set metadata
...
- Persists records to `teacher_feedback_records.json` in a school/teacher hierarchy.
...
5. Results are displayed in the frontend; evaluation details are stored in `question_evaluations.json`.
